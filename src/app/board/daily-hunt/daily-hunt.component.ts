  ...
  implements OnInit, OnDestroy, OnChanges, AfterViewInit
{
  ...
  private readonly svgTemplate = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1080 1080" preserveAspectRatio="xMidYMid meet">
  <g transform="translate(0.000000,1080.000000) scale(0.100000,-0.100000)" fill="{color}" stroke="none">
  ...
  private updateSvgColorAndRedraw() {
    // 1. Obtenir la couleur actuelle en fonction du theme
    const color = this.themeService.isDarkMode ? '#c0c6dd' : '#2a3042';

    // 2. Mettre à jour le SVG avec cette couleur
    const coloredSvg = this.svgTemplate.replace('{color}', color);
    ...
    img.onload = () => {
      ...
      this.draw(); // Une fois l'image chargée, on la dessine
      console.log(`Image déssinée avec la couleur ${color}`);
    };
    img.onerror = () => {
  ...
  private draw() {
    ...
    // Si la chasse est trouvée ou si l'image n'est pas encore chargée, ne rien dessiner
    if (this.huntFound || !this.svgImage) {
      return;
    }
